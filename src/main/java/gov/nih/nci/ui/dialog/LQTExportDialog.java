...
public class LQTExportDialog extends JPanel {
     ...
     */
    public LQTExportDialog(OWLEditorKit editorKit, String customText, List<OWLEntity> output, boolean allowOutputModifications) {
    ...
    private void invokeAxiomAnnotationHandler() {
        Object obj = propertiesList.getSelectedValue();
        if (obj instanceof OwlEntityListItem)
        addDepProps(((OwlEntityListItem)obj).getEntity());
    ...
    private void exportToCsv() throws IOException {
        LQTExporter lqtExporter = new LQTExporter(editorKit, selectedFile, output, getEntities(propertiesList), dependentAnnotations, fileDelim.getText(), 
        		propertyValuesDelim.getText(), includePropertyNames.isSelected(), includeEntityTypes.isSelected(),
                useCurrentRendering.isSelected(), includeSuperclasses.isSelected(), includeCustomText.isSelected(), customText);
    ...
    public static boolean showDialog(OWLEditorKit editorKit, String customText, List<OWLEntity> results, boolean allowOutputAlterations) throws IOException {
        LQTExportDialog panel = new LQTExportDialog(editorKit, customText, results, allowOutputAlterations);
